...
class LatinoToJs(LatinoGrammarListener):
    ...
    def enterSentence(self, ctx: LatinoGrammarParser.SentenceContext):
        ...
        # TODO: this needs to be compared
        #if not self.infor:
            #if self.indentationStack:
                #for i in range(len(self.indentationStack)):
                    #self.jsCode += '    '
        # TODO: compare the block below with the commented one above
        if self.indentationStack:
            for i in range(len(self.indentationStack)):
                self.jsCode += '    '
        ...
        if ctx.assig():
            ...
            # TODO: this seems to be the only changes here, test
            if not self.infor:
